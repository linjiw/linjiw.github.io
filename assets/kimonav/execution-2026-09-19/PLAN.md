...
**Latest fixed-candidate qualification:** Foot-pose constrained lowering completes 3/3 beam and 3/3 clear tasks; matched local duck is 2/3 beam. This qualifies a fixed development candidate, not an online replanner or a new public-selector result. The new reference passes the retained difficult seed; next test execution-qualified public selection and a simpler matched posture edit before crediting online replanning. [Complete results](/kimonav/lowering/).
...
The registered experiments now provide a limited motor baseline, typed contacts, an initial goal/map selector and a stronger fixed lowering candidate that rescues its retained difficult scene. The immediate next deliverable is execution-qualified public candidate selection, followed by a frozen clearance-variation comparison against the original selector and improved fixed reference. A simpler matched posture edit remains a mechanism control. Online reference replacement is reserved for a remaining failure that these stronger baselines cannot resolve; it still requires qualified seams and original-clock preservation. Public selection of the new candidate, observation robustness and broader scene generalization remain pending. The long-term goal stays stable while the method remains replaceable.
