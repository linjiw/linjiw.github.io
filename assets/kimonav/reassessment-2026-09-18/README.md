...
Research reassessment, 18 September 2026. This is the current strategic plan; earlier experiment protocols and results remain historical records. This review adds no training, physics trials, or hardware runs.
...
The numerical budgets below are proposed development allocations, not completed studies or a final preregistration. Freeze exact programs, seeds, thresholds and analysis before execution. Use the existing flat floor and motions for the first two stages; reuse existing scene data at the geometry stage. Do not start another open-ended architecture search.
...
Initial development budget: six command designs × three execution seeds × two motors, 36 trials. This establishes feasibility and losing subgroups, not broad generalization. If neither motor can execute the necessary reference from the intended starts, work on reference construction, transitions or motor adaptation before student compression. If the task is repeatedly infeasible, reject that command or propose an explicit new task version; do not manufacture labels for it.

### 2. Resolve the sparse-control failure with one bounded comparison
...
The immediate deliverable after this review is a **registered capability-and-baseline experiment**, with executable input/output adapters, feasible commands, paired motors, explicit start conditions and an independent scoring script. The decision it changes is whether to invest next in planning/geometry, motor support, or student compression. The long-term goal stays stable while the method remains replaceable.
