...
let project=await fetch('project.json').then(r=>r.json()), cues=await fetch('assets/cues.json').then(r=>r.ok?r.json():[]).catch(()=>[]);
...
function pause(){playing=false;audio.pause();$('play').textContent='▶ Play';document.querySelectorAll('#media-layer video').forEach(v=>v.pause())}
function setTime(v){t=Math.max(0,Math.min(project.duration-.001,v));if(Number.isFinite(audio.duration))audio.currentTime=t;draw(true)}
async function play(){window.LUCID?.motion?.stop();$('library-video')?.pause();$('parallel-video')?.pause();$('process-film')?.pause();$('hardware-video')?.pause();if(t>=project.duration-.1)setTime(0);playing=true;last=performance.now();$('play').textContent='Ⅱ Pause';$('start-overlay').hidden=true;if(sound&&!narrationDirty){audio.currentTime=t;audio.playbackRate=speed;try{await audio.play()}catch{status('Playback started without audio. Use Sound on to try again.')}}draw(true)}
...
function draw(force=false){let s=scene();$('scene').innerHTML=renderSVG(t,project,{cues,captions:cc});$('seek').value=t;$('time').textContent=`${fmt(t)} / ${fmt(project.duration)}`;initMedia(s,force);
 if(s.id!==currentScene||force){currentScene=s.id;$('chapter-kicker').textContent=s.eyebrow;$('chapter-title').textContent=s.title;$('chapter-desc').textContent=descriptions[s.id];document.querySelectorAll('#chapters button').forEach((b,i)=>{b.classList.toggle('active',project.scenes[i].id===s.id);b.classList.toggle('past',project.scenes[i].end<=t)})}}
function tick(now){if(playing){let dt=(now-last)/1000;last=now;if(sound&&!narrationDirty&&!audio.paused&&!audio.ended)t=audio.currentTime;else t+=dt*speed;if(t>=project.duration){t=project.duration-.001;pause()}draw()}requestAnimationFrame(tick)}
function chapters(){
 $('chapters').innerHTML='';$('edit-scene').innerHTML='';project.scenes.forEach((s,i)=>{let b=document.createElement('button');b.style.flex=s.end-s.start;b.setAttribute('aria-label',`${fmt(s.start)} ${s.title}`);b.onclick=()=>setTime(s.start);$('chapters').append(b);let o=document.createElement('option');o.value=i;o.textContent=`${fmt(s.start)} · ${s.title}`;$('edit-scene').append(o)})
}
...
$('play').onclick=()=>playing?pause():play();$('start-overlay').onclick=play;
$('seek').oninput=e=>setTime(Number(e.target.value));$('back').onclick=()=>{let i=project.scenes.indexOf(scene());setTime(project.scenes[Math.max(0,i-1)].start)};$('forward').onclick=()=>{let i=project.scenes.indexOf(scene());setTime(project.scenes[Math.min(project.scenes.length-1,i+1)].start)};
$('sound').onclick=()=>{sound=!sound;$('sound').textContent=sound?'Sound on':'Muted';$('sound').setAttribute('aria-pressed',String(sound));if(!sound)audio.pause();else if(playing&&!narrationDirty){audio.currentTime=t;audio.play().catch(()=>{})}};
$('captions').onclick=()=>{cc=!cc;$('captions').setAttribute('aria-pressed',String(cc));draw()};$('speed').onchange=e=>{speed=Number(e.target.value);audio.playbackRate=speed};
...
document.addEventListener('keydown',e=>{if(['INPUT','TEXTAREA','SELECT','VIDEO'].includes(e.target.tagName))return;if(e.code==='Space'){e.preventDefault();playing?pause():play()}if(e.code==='ArrowRight'){e.preventDefault();setTime(t+5)}if(e.code==='ArrowLeft'){e.preventDefault();setTime(t-5)}if(e.code==='Escape')document.body.classList.remove('cinema')});
$('brand').onclick=e=>{e.preventDefault();pause();setTime(0)};
window.addEventListener('beforeunload',e=>{if(dirty){e.preventDefault();e.returnValue=''}});
window.LUCID={seek:setTime,play,pause,render:draw,get time(){return t},get project(){return project}};
